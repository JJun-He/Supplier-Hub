...
## 2026-09-17 — 후속 리뷰의 설계 근거·의존성 보완

후속 리뷰에서 연결·응답 제한 값의 선택 이유가 부족하다는 지적을 확인했다. 현재 초기값을 유지하는 이유와 짧게/길게 설정할 때의 비용을 보완했다. 응답 제한은 네트워크 읽기 사이의 간격이며 호출 제한과 합산하는 시간이 아니다. 합의하지 않은 고객 응답 목표나 측정하지 않은 네트워크·파싱 시간으로 수치를 도출했다고 설명하지 않는다. 이번 검토의 보완이며 과거부터 충분히 설명했던 것으로 기록하지 않는다.

서킷 브레이커는 Supplier·작업별 표본·실패/느린 호출 기준, 차단·복구, 부분 응답·카탈로그 재시도 관계를 갖춘 제안 설계로 정리했다. 코드·라이브러리·실패 유형은 추가하지 않았다. 공개 문서의 외부 문서 페이지 색인을 제거하고 구현·검증 근거는 유지했다.
