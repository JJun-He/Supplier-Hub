...
> 후속 상태(2026-09-16): 8-A에서 C-01~04, E-01/E-03/E-06, B 실패 매핑과 오류 표 회귀 검증을 반영했다. [설계 §16](architecture-decisions.md#16-8-a-입력중복예외-계약-보완)과 [구현 진행표](implementation-plan.md#64-8단계-진행-상태)를 참고한다. 아래 분석은 감사 기준 커밋 당시 기록이며, 8-B 자원 제한·관측성은 [설계 §17](architecture-decisions.md#17-8-b-자원-제한과-업무-지표)에서 완료 상태를 확인한다. 8-C 동기화 보호와 DB 예산은 [설계 §18](architecture-decisions.md#18-8-c-동기화-중복-실행과-db-시간-예산)에서 완료 상태를 확인한다. 8-D의 실제 프로세스 E2E 9개도 완료했다([실행 결과](e2e-verification.md)).
...
> 현재 변경 경계: 8-B의 구체 자원·지표 구현 결합은 [설계 §17.1](architecture-decisions.md#171-호출-수와-연결-풀), 8-C의 deadline·읽기 실패 포트와 JPA/JDBC 어댑터 책임은 [설계 §18.3](architecture-decisions.md#183-실패와-고객-응답)에 기록한다. 아래 표는 감사 시점 의존 관계를 보존한다.
