...
| 시간 설정 | 검색 | 카탈로그 |
| --- | ---: | ---: |
| 연결 timeout | 500ms | 500ms |
| 응답 timeout | 2초 | 3초 |
| 호출 전체 timeout | 3초 | 4초 / 시도 |
| 검색 전체 예산 | 5초 | 적용 안 함. retry·저장은 별도 경계 |

이 값은 초기값이며 운영 p95·p99 지연과 고객 목표 시간을 관측하여 조정한다. 카탈로그 한 시도의 timeout이 재시도·backoff·DB 저장까지 합친 동기화 전체 상한은 아니다.
...
| 검토 대상 | 도입 조건과 우선 접근 |
| --- | --- |
| 서킷 브레이커 | 반복 장애가 관측되고 빠른 차단이 호출 한도·고객 지연에 이점이 있을 때. Supplier·작업별 실패/지연 기준, open 기간과 half-open 탐색 수를 정하고 부분 응답·카탈로그 retry와 상호작용 검증 |
...
캐시·서킷 브레이커의 수치·라이브러리를 현재부터 고정하지 않는다. 캐시는 실패한 Supplier의 값을 조용히 최신 결과처럼 반환하는 정책과 다르므로, 오래된 값을 허용하면 응답에 그 의미를 표현해야 한다. 서킷 브레이커도 로컬 용량 부족·잘못된 요청을 외부 장애로 합산할지 명확히 구분해야 한다.
