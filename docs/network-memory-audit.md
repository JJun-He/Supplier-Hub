...
> 후속 상태(2026-09-16): 8-B에서 N-01/N-02/N-04의 호출 한도·응답 크기·업무 지표를 보완하고 HTTP 회귀 검증을 추가했다. [설계 §17](architecture-decisions.md#17-8-b-자원-제한과-업무-지표)을 참고한다. DB 시간 예산은 8-C에서 보완했다([설계 §18](architecture-decisions.md#18-8-c-동기화-중복-실행과-db-시간-예산)). 전체 연결 검증은 8-D에서 완료했다([실행 결과](e2e-verification.md)). 아래 수치와 미구현 표현은 감사 당시 기록이다.
