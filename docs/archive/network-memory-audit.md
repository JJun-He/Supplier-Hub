...
> 후속 상태(2026-09-16): 호출 자원 보완에서 N-01/N-02/N-04의 호출 한도·응답 크기·업무 지표를 보완하고 HTTP 회귀 검증을 추가했다. [설계](../architecture-decisions.md)를 참고한다. DB 시간 예산은 DB 정합성·시간 예산 보완에서 보완했다([설계](../architecture-decisions.md)). 실제 프로세스의 전체 연결 검증도 완료했다([실행 결과](../e2e-verification.md)). 아래 수치와 미구현 표현은 감사 당시 기록이다.

## 1. 결론과 범위

- 작성일: 2026-09-15
- 기준 커밋: `eff5bb83a940d8d29a778b9de0d041d888771388`
- 범위: 네트워크 감사. 외부 호출 수, 동시 검색, timeout과 연결 종료, 응답 크기, 메모리 할당, 관측성
...
**요청별 동시 호출 제한과 timeout 취소는 동작한다. 다만 동시 고객 수가 늘면 Supplier 호출도 함께 늘며, 정상 형식의 카탈로그·검색 응답이 현재 256 KiB 버퍼 한도를 초과해 실패했다. 견고성 보완은 전역 자원 제한, 응답 크기 정책, 업무 결과 지표를 먼저 보완하는 것이 적절하다.**
...
`bodyToMono(응답 DTO)`는 전체 응답을 조립한다. 숙소를 50개로 나누어도 객실 수·요금 옵션 수·박수에 따라 응답 크기가 달라진다. 위 검색은 현재 허용하는 30박과 정확히 50개 숙소를 사용했다. 수천 개 카탈로그의 처리 한계와 함께 견고성 보완에서 해결할 가치가 높다.
...
견고성 보완에서는 전역 동시성 제한과 시간 예산으로 이 잔여 작업의 비용을 먼저 제한한다. 고객 연결 종료를 이유로 MVC+JPA 전체를 WebFlux로 바꾸는 것은 현재 근거에 비해 변경 범위가 크다. 실제 배포 경로의 연결 종료 감지와 취소 연동은 후속 검증으로 남길 수 있다.
...
고객 API 구현 때 전체 카탈로그를 결과에 복사하던 문제는 제거됐다. 그러나 요청 조립에 필요한 원본 rows, 정렬 목록, grouped mapping은 요청마다 만들어진다. 실제 Offer가 0개여도 이 비용은 남는다. 동시 고객 제한과 함께 평가할 대상이다.
...
- 임시 테스트 중 일부는 예외를 포괄적으로 assert하고 세부 종류·수치는 로그로 관측했다. 견고성 보완 정식 회귀 테스트에서는 필요한 종류·한도·반환 후 회복까지 명시적으로 assert한다.
- 다음 구조 감사는 API/application/domain/infrastructure 경계, SOLID, Supplier 추가 시 변경 범위, 검증·예외의 중복과 누락, 테스트 분류다. 이후 전체 감사 결과를 하나의 견고성 보완 구현 목록으로 확정한다.
