...
> 후속 상태(2026-09-16): 입력·예외 보완에서 C-01~04, E-01/E-03/E-06, B 실패 매핑과 오류 표 회귀 검증을 반영했다. [설계](../architecture-decisions.md)와 [구현 진행표](implementation-plan.md)를 참고한다. 아래 분석은 감사 기준 커밋 당시 기록이며, 자원 제한·관측성은 [설계](../architecture-decisions.md)에서 완료 상태를 확인한다. 동기화 보호와 DB 예산은 [설계](../architecture-decisions.md)에서 완료 상태를 확인한다. 전체 연결 검증의 실제 프로세스 E2E 9개도 완료했다([실행 결과](../e2e-verification.md)).

## 1. 결론과 검증 수준

- 작성·재검토일: 2026-09-16
- 기준 커밋: `eff5bb83a940d8d29a778b9de0d041d888771388`
- 범위: 구조 감사. 계층 경계, SOLID 5원칙, Supplier 추가 비용, 검증·예외, 테스트 책임과 누락
- 이번 방법: 제품·테스트 소스 재검토, 기존 실험 기록 대조, 임시 경계 검증 5개와 실제 Boot/Tomcat HTTP 검증 1개
- 이번 실행: **6개 성공, 실패·오류·건너뜀 0개**. 제품 코드·설정·정식 테스트는 변경하지 않았다.

**공급사별 어댑터와 공통 검색 로직의 분리는 적절하다. 견고성 보완에서는 카탈로그의 client/snapshot 계약 검사, 입력 거부와 내부 오류의 구분, 실제 연동 검증을 먼저 보완한다. JPA Entity 분리·검증 유틸 통합·컨테이너 단일화는 필수 수정으로 올리지 않는다.**
...
| HTTP 테스트가 연결 종료까지 검증 | 정식 client 테스트는 상대 연결 종료를 assert하지 않는다. 해당 근거는 별도의 네트워크 감사 임시 검증이다. |
| PostgreSQL 테스트라 실행 계획도 동일 | 같은 DB 계열을 검사하는 장점이 있다. 데이터·통계·환경이 다르면 계획은 달라진다. 실제 계획 측정은 DB 감사에 별도로 있다. |
...
| ID | 판단 | 내용 | 권장 시점 |
| --- | --- | --- | --- |
| E-06 (추가) | P2·확장 시 계약 방어 누락 | 카탈로그 client 누락·중복, snapshot의 Supplier 불일치를 검사하지 않음 | 입력·예외 보완 |
| E-01 / E-03 | P2·진단 구분 부족 | adapter 내부 오류는 UNKNOWN, mapper NPE는 항목 거부로 집계 | C-01/C-02와 함께 입력·예외 보완 |
| E-02 | P2·관측성 | 카탈로그 일반 실패 로그는 있지만 실패 지표·갱신 신선도 신호 없음 | N-04와 함께 호출 자원 보완 |
| T-01 / T-02 | P2·검증 공백 | 오류 표와 실제 DB→Mock→검색 API 흐름의 정식 회귀 검증 부족 | 입력·예외 보완 / 전체 연결 검증 |
...
> 현재 변경 경계: 호출 자원 보완의 구체 자원·지표 구현 결합은 [설계](../architecture-decisions.md), DB 정합성·시간 예산 보완의 deadline·읽기 실패 포트와 JPA/JDBC 어댑터 책임은 [설계](../architecture-decisions.md)에 기록한다. 아래 표는 감사 시점 의존 관계를 보존한다.
...
카탈로그의 활성 상태·누락 횟수·내부 ID는 영속 상태다. 반면 검색 Offer는 매 요청의 계산 결과다. 서로 다른 모델을 택할 이유가 있으며, JPA annotation의 존재만으로 SOLID 위반이라고 할 수 없다. 최종 문서·실행 검증 문서에는 이 선택과 search domain에 영속 의존을 도입하지 않는 현재 경계를 설명하면 된다.
...
| `application.yaml` | C 설정 블록 4줄 | 기존 diff 확인 |
...
입력·예외 보완에서는 데이터 표를 순회하는 parameterized test가 적절하다. B 본문 코드를 공통 HTTP 매퍼 테스트에 넣으면 실제 분기를 검사하지 못한다. 작은 B 매퍼로 모으거나 두 실제 client 경로에 같은 표를 적용한다.

### 6.3 T-02 / T-03: 위험에 따라 보강할 사례

| 우선 | 사례 | 넣을 테스트 경계 |
| --- | --- | --- |
| 입력·예외 보완 | 소수·overflow 금액/인원, null·잘못된 날짜와 정상 형제 항목 | 실제 주입 codec + adapter; domain 값 타입만 생성해서 대체하지 않음 |
| 입력·예외 보완 | 동일 Offer 중복·충돌 정책, 특수 코드 URI 보존 | adapter/집계의 계약 경계 |
| 입력·예외 보완 | catalog client 누락·중복·snapshot Supplier 불일치 | 동기화 application 단위 |
| 입력·예외 보완 | HTTP/B 본문 코드 표, timeout 원인 체인, 내부 오류와 항목 거부 | 해당 매퍼·normalizer·서비스 |
| DB 정합성·시간 예산 보완 | 독립 commit 후 ID 유지, 중간 저장 실패 rollback, 허용하는 동기화 경쟁 | 실제 PostgreSQL, 테스트 전체 자동 rollback과 분리 |
| 호출 자원 보완 | 여러 고객 전역 한도, 취소 후 허용량 반환, 한 Supplier 포화 격리 | 실제 HTTP + 서비스 공유 인스턴스 |
| 호출 자원 보완 | 정상 큰 응답·한도 초과·취소 후 회복 | 실제 WebClient codec/연결 |
| 전체 연결 검증 | DB 매핑 동기화→두 Mock→실제 검색 API의 정상/부분/전체 실패 | 실제 애플리케이션 설정을 연결한 E2E |
...
따라서 이번 견고성 보완 필수 작업에서 제외한다. 실제 실행 시간 비중을 측정한 뒤 공유 전후의 독립성·시간을 비교할 때 다시 검토한다.

## 7. 서비스 요구사항과 현재 증거

정확성 요구사항 대조표와 현재 구현·테스트를 기준으로 정리했다.

| 중점 | 현재 근거 | 남은 증거 |
| --- | --- | --- |
| 같은 Supplier 상품의 내부 ID 안정성 | writer/repository 테스트 + DB 감사 독립 commit 검증 | 지원할 동시 실행 모델 확정과 회귀 테스트 |
...
| timeout·병렬·부분 실패·B 본문 실패 | 검색/client 테스트 + 네트워크 감사 HTTP 취소 검증 | 전역 한도·크기 정책·완전 연결 E2E |
| 핵심 흐름을 끊김 없이 실행 | 각 계층별 근거 존재 | 실제 DB→Mock→고객 응답의 일관된 실행 증거 |
| 설명·실행 재현 | 설계 기록·감사 문서·JOURNAL | 최종 문서·실행 검증 README, 장애 재현 명령, 한계·trade-off 정리 |

현재 핵심 구현은 있지만 미수정 정확성 문제와 E2E 검증 공백이 남아 있다. SOLID 순수성을 높이는 리팩터링보다 이 항목을 먼저 완료한다.

## 8. 다음 구현 순서 — 네 감사 결과 통합

아래 정확성·자원·DB·전체 연결 보완은 이번에 정리한 **견고성 보완 내부 작업 묶음**이다. 기존 단계 계획을 이미 구현했거나 대체 완료했다는 뜻은 아니다.

| 묶음 | 최소 범위 | 완료 증거 |
| --- | --- | --- |
| **정확성·입력/예외 계약** | C-01~04, E-06, E-01/E-03, B 오류 표 및 필요한 fixture 정리 | 엄격 숫자·항목 격리·중복 정책·URI·client/snapshot 계약·오류 분류 회귀 |
| **자원 제한·관측성** | N-01/N-02/N-04, HTTP pool/대기 예산, E-02 | 동시 고객에서도 한도, 큰 정상 응답 수용/초과 분류, 취소 후 회복, 업무 실패 지표 |
| **DB 정합성·시간 예산 정합성·시간 예산** | D-01 지원 범위 결정과 필요한 보호, D-02 timeout; 이때 필요한 writer 경계 정리 | 독립 commit/rollback/경쟁 테스트. 필요하면 갱신 batch 검증. 인덱스는 측정 근거가 있을 때만 |
| **전체 연결 검증 전체 연결·마지막 검증** | 실제 Mock 정상/부분/전체 실패, timeout, 전체 정식 테스트 | 서로 연결된 실행 증거와 회귀 통과 |
| **최종 문서·실행 검증 문서화** | README, 실행/장애 명령, 설계 결정, 확장 절차, 미구현 한계 | 요구사항→구현→테스트→문서 최종 대조 |

입력·예외 보완의 입력 파싱, 오류 분류, 오류 표를 함께 다루면 같은 adapter를 반복해서 다시 고치는 일을 줄인다. 호출 자원 보완/DB 정합성·시간 예산 보완의 시간 예산도 한 정책으로 맞춘다. DB의 심각한 정합성 결함을 지원 환경에서 확인하면 자원 최적화보다 먼저 수정한다.
