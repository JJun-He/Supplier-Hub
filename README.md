...
목록 동기화부터 검색 API까지 기본 구현을 마쳤습니다. 중간 기술 감사에서 발견한 정확성·자원 제한·예외 경계 문제는 수정 예정입니다. 이 문서는 초기 실행 안내이며, 전체 연결 검증과 최종 문서 정리는 후속 단계에 진행합니다.
...
기존 전체 검증 기준점은 메인 89개·Mock 6개 통과입니다. 테스트 분류와 이후 임시 감사 검증은 [구조 감사](docs/structure-audit.md)에 구분해 기록했습니다.
...
1. **8-A:** 엄격한 숫자 파싱, 항목별 오류 격리, Offer 중복 정책, URI 값 보존, 카탈로그 계약 검사
2. **8-B:** Supplier별 전역 호출 제한, 응답 크기 정책, 업무 실패 지표
3. **8-C:** 동기화 동시 실행 지원 범위와 DB 시간 예산
4. **8-D:** 실제 DB·Mock·고객 API 전체 연결 검증
5. **9단계:** 실행 절차 재검증과 최종 설계·운영 한계 문서화

현재 호출 동시성 4개 제한은 고객 요청마다 적용됩니다. 정상적인 큰 응답이 codec 한도를 넘는 사례, 숫자 변환과 중복 Offer 등의 문제도 아직 남아 있습니다. 감사 보고서는 해결 목록이며 완료 보고서가 아닙니다.
